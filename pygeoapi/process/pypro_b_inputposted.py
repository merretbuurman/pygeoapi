...
# Read the data to be posted from file: Argument list is too long! Only works with short version
# Note: The awk command is needed to replace the newlines with \n characters:
curl -X POST "http://130.225.37.27:5000/processes/assessment-indicator-posted/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"assessmentPeriod\": \"2011-2016\", \"annual_indicators_csv\": \"$(awk '{printf "%s\\n", $0}' /tmp/Annual_Indicator.csv)\"}}"
curl -X POST "http://130.225.37.27:5000/processes/assessment-indicator-posted/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"assessmentPeriod\": \"2011-2016\", \"annual_indicators_csv\": \"$(awk '{printf "%s\\n", $0}' ../../example_results/a_Annual_Indicator_short.csv)\"}}"
...
class HELCOMAssessmentIndicatorPostedProcessor(BaseProcessor):
    ...
    def _execute(self, data):
        ...
        r_args = [configurationFileName, input_temp_path, path_intermediate, resultfilepath]
