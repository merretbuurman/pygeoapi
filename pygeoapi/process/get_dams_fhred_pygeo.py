...
from pygeoapi.process.aquainfra.aquainfra import _read_dam_data_FHReD
...
Curl to test:
curl -X POST "http://localhost:5000/processes/get-dams-fhred/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"basin_id\": \"481051\"}}"

# With FeatureCollection
curl -X POST "http://localhost:5000/processes/get-dams-fhred/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"basin_geojson\": {\"type\": \"FeatureCollection\", \"features\": [{\"type\": \"Feature\", \"properties\": {}, \"geometry\": {\"type\": \"Polygon\", \"coordinates\": [[[-43.8798, -16.701], [ -46.5404,-16.701], [-46.5404, -18.608], [-43.87982, -18.608], [-43.879819, -16.701]]]}}]}}}"

# With Feature
curl -X POST "http://localhost:5000/processes/get-dams-fhred/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"basin_geojson\": {\"type\": \"Feature\", \"properties\": {}, \"geometry\": {\"type\": \"Polygon\", \"coordinates\": [[[-43.8798, -16.701], [ -46.5404,-16.701], [-46.5404, -18.608], [-43.87982, -18.608], [-43.879819, -16.701]]]}}}}"

# With Polygon
curl -X POST "http://localhost:5000/processes/get-dams-fhred/execution" -H "Content-Type: application/json" -d "{\"inputs\":{\"basin_geojson\": {\"type\": \"Polygon\", \"coordinates\": [[[-43.8798, -16.701], [ -46.5404,-16.701], [-46.5404, -18.608], [-43.87982, -18.608], [-43.879819, -16.701]]]}}}"
...
PROCESS_METADATA = {
    'version': '0.0.1',
    'id': 'BLA',
    'title': {'en': 'Get dams from FHReD'},
    'description': {
        'en': 'Retrieving dam data originating from FHReD database, for one drainage basin. '
    ...
    'inputs': {
        'basin_id': {
            'title': 'Basin ID',
            'description': 'ID of the drainage basin in which you would like the dams.',
            'schema': {'type': 'string'},
            'minOccurs': 0,
        ...
        'basin_geojson': {
            ...
            'description': 'Drainage Basin Polygon in which you would like the dams as GeoJSON',
            'schema': {
                'type': 'object',
...
class DamFhredProcessor(BaseProcessor):

    def __init__(self, processor_def):
         super().__init__(processor_def, PROCESS_METADATA)

    def execute(self, data):
        LOGGER.info('Starting DamFhredProcessor as ogc_service!"')
        ...
        # Get dam data
        LOGGER.info('Retrieving data about dams from FHReD...')
        dams_fhred = _read_dam_data_FHReD(DATA_DIR)
        LOGGER.info('Retrieving data about dams from FHReD... done.')

        # Filter by basin polygon:
        LOGGER.info('Filtering dams by drainage basin "%s...' % basin_id)
        dams_fhred = _spatial_filter(dams_fhred, polygon_geodataframe)
        LOGGER.info('Filtering dams by drainage basin... done. %s objects left...' % len(dams_fhred))
        #print(dams_fhred.head())

        # Convert to geojson:
        output_as_geodataframe = dams_fhred
    ...
    def __repr__(self):
        return f'<DamFhredProcessor> {self.name}'
